...
export default function AddStudent({ close, editData }) {
  ...
  const [form, setForm] = useState({
    ...
    fatherName: "", motherName: "", aadhaar: "", gender: "", category: "", dob: "", session: "", photo: null, photoURL: "",
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const qReg = query(collection(db, "students"), orderBy("regNo", "desc"), limit(1));
        ...
        if (isMounted) {
          ...
          if (configSnap.exists()) {
            ...
            const classes = Object.keys(data.mapping || {}).sort((a, b) => 
          ...
          if (!editData) {
            const now = new Date();
            const session = now.getMonth() + 1 >= 4 ? `${now.getFullYear()}-${(now.getFullYear()+1).toString().slice(-2)}` : `${now.getFullYear()-1}-${now.getFullYear().toString().slice(-2)}`;
            setForm(prev => ({ ...prev, regNo: nextReg, session }));
          }
        }
  ...
  // 2. Live Roll Number Logic
  useEffect(() => {
    if (!form.className || !form.session || editData) return;
    ...
    fetchRoll();
  }, [form.className, form.session, editData]);
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    let existingTokens = []; // Tokens store karne ke liye variable

    if (!pId) {
      // Naya parent create ho raha hai - empty array bhejenge
      const pDoc = await addDoc(collection(db, "parents"), {
        ...
        students: [],
        fcmTokens: [], // Initial empty array
        createdAt: serverTimestamp()
      });
      pId = pDoc.id;
    } else {
      // Agar parent existing hai, to unke tokens nikaalte hain
      const parentRef = doc(db, "parents", pId);
      const parentSnap = await getDoc(parentRef);
      if (parentSnap.exists()) {
        existingTokens = parentSnap.data().fcmTokens || [];
      }
    }
    ...
    if (editData) {
      await updateDoc(doc(db, "students", editData.id), { 
        ...
        parentId: pId, 
        subjects,
        fcmTokens: existingTokens // Update case mein bhi sync rakhein
      });
      close();
    } else {
      const sDoc = await addDoc(collection(db, "students"), {
        ...
        parentId: pId,
        subjects,
        fcmTokens: existingTokens, // Yahan tokens array insert ho gaya
        attendance: months.reduce((acc, m) => ({ ...acc, [m]: { present: 0, absent: 0 } }), {}),
        createdAt: serverTimestamp(),
        deletedAt: null
      });

      await updateDoc(doc(db, "parents", pId), { 
      ...
      {!showPrint ? (
        <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[95vh] overflow-hidden animate-in zoom-in duration-300">
          ...
          <div className="p-5 border-b flex justify-between items-center bg-blue-700 text-white">
            <h2 className="text-xl font-black tracking-tight">{editData ? t.editTitle : t.title}</h2>
          ...
          <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-6">
            ...
            <div className="flex flex-col items-center bg-blue-50/30 p-4 rounded-3xl border-2 border-dashed border-blue-100">
              <div className="relative group">
                ...
                <label className="absolute -bottom-2 -right-2 bg-blue-600 text-white p-2 rounded-full cursor-pointer shadow-lg hover:bg-blue-800">
            ...
            <section className="grid md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2 grid grid-cols-2 gap-3 mb-2">
                <div className="bg-blue-50 p-3 rounded-2xl border-2 border-blue-100 flex flex-col items-center justify-center">
                  <span className="text-[9px] font-black text-blue-400 uppercase tracking-tighter">Registration Number</span>
                  <span className="text-xl font-black text-blue-800">{form.regNo}</span>
                </div>
                <div className="bg-green-50 p-3 rounded-2xl border-2 border-green-100 flex flex-col items-center justify-center">
                  <span className="text-[9px] font-black text-green-400 uppercase tracking-tighter">Roll Number</span>
                  <span className="text-xl font-black text-green-800">{form.rollNumber}</span>
                </div>
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">{t.admDate}</label>
                <input type="date" name="admissionDate" value={form.admissionDate} onChange={handleChange} className="border-2 p-2.5 rounded-xl focus:border-blue-500 outline-none" required />
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">{t.name}</label>
                <input name="name" value={form.name} onChange={handleChange} placeholder="Full Name" className="border-2 p-2.5 rounded-xl focus:border-blue-500 outline-none font-bold" required />
              </div>

              {/* MOTHER NAME - INTEGRATED */}
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">{t.motherName}</label>
                <input name="motherName" value={form.motherName} onChange={handleChange} placeholder="Mother's Name" className="border-2 p-2.5 rounded-xl focus:border-blue-500 outline-none font-bold" required />
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">Class (From Config)</label>
                <select name="className" value={form.className} onChange={handleChange} className="border-2 p-2.5 rounded-xl focus:border-blue-500 outline-none font-black text-blue-800" required>
              ...
              <div className="flex flex-col gap-1">
                ...
                <select name="gender" value={form.gender} onChange={handleChange} className="border-2 p-2.5 rounded-xl outline-none focus:border-blue-500 font-bold" required>
                  <option value="">Gender</option><option>Male</option><option>Female</option>
                </select>
              </div>

              {/* DOCUMENT CHECKLIST - INTEGRATED */}
              <div className="md:col-span-2 space-y-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {[
                    ...
                    { key: "docMarksheet", label: "Marksheet" },
                    { key: "docTC", label: "T.C." },
                    { key: "docPhoto", label: "Photos" }
                  ].map((doc) => (
                    <label key={doc.key} className="flex items-center gap-2 bg-gray-50 p-2.5 rounded-xl border border-gray-100 cursor-pointer hover:bg-blue-50">
              ...
              <div className="md:col-span-2 bg-blue-50/50 p-4 rounded-3xl border-2 border-dashed border-blue-100">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {subjects.map((sub, i) => (
                    <span key={i} className="bg-white border-2 border-blue-100 text-blue-700 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase shadow-sm flex items-center gap-2">
                      {sub} <button type="button" onClick={() => setSubjects(s => s.filter((_, idx) => idx !== i))} className="text-red-400 hover:text-red-600 transition-colors">×</button>
                    </span>
                  ))}
                </div>
                <div className="flex gap-2">
                  <select 
                    ...
                    className="flex-1 text-sm p-2.5 rounded-xl border-none ring-1 ring-blue-200 outline-none focus:ring-2 focus:ring-blue-500"
                  >
              ...
              <div className="md:col-span-2 grid grid-cols-2 gap-4 bg-purple-50 p-4 rounded-3xl border border-purple-100">
                <div className="flex flex-col gap-1">
                    ...
                    }} className="border-2 p-2.5 rounded-xl outline-none focus:border-purple-500 font-bold">
                ...
                <div className="flex flex-col gap-1 text-center">
                    ...
                    <div className="bg-white text-purple-700 font-black p-2.5 rounded-xl border-2 border-purple-200 shadow-inner text-lg">₹ {form.busFees}</div>
                </div>
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">Category</label>
                <select name="category" value={form.category} onChange={handleChange} className="border-2 p-2.5 rounded-xl outline-none focus:border-blue-500 font-bold" required>
            ...
            <section className="space-y-4">
              ...
              <div className="relative">
                <div onClick={() => setFatherOpen(!fatherOpen)} className="border-2 border-green-100 p-4 rounded-2xl cursor-pointer bg-green-50/20 flex justify-between items-center text-xs font-black text-green-700 hover:border-green-400 transition-all">
                  <span>{form.parentId ? `✅ LINKED: ${form.fatherName}` : "🔍 SEARCH PARENT DATABASE"}</span>
                  <span className={fatherOpen ? "rotate-180 transition-transform" : ""}>▼</span>
                </div>
                {fatherOpen && (
                  <div className="absolute z-30 bg-white border-2 w-full mt-2 rounded-3xl shadow-2xl max-h-60 overflow-auto animate-in slide-in-from-top-2">
                    ...
                    {/* NEW PARENT OPTION - INTEGRATED */}
                    <div onClick={() => {
                        setForm(p => ({ ...p, parentId: "", fatherName: "", motherName: "", phone: "", address: "" }));
                        setFatherOpen(false);
                      }} className="p-4 bg-blue-50 text-blue-700 font-black text-center cursor-pointer hover:bg-blue-100 border-b italic">
                    ...
                    {parents.filter(p => p.fatherName?.toLowerCase().includes(fatherSearch.toLowerCase()) || p.phone?.includes(fatherSearch)).map(p => (
                      <div key={p.id} onClick={() => {
                        setForm(prev => ({ ...prev, fatherName: p.fatherName, motherName: p.motherName || "", phone: p.phone, parentId: p.id, address: p.address || prev.address }));
                        setFatherOpen(false);
                      }} className="p-4 hover:bg-green-50 cursor-pointer border-b text-sm flex justify-between items-center">
                        <span className="font-bold text-gray-700">{p.fatherName}</span>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <input name="fatherName" value={form.fatherName} onChange={handleChange} placeholder="Father's Name" className="border-2 p-2.5 rounded-xl outline-none focus:border-blue-500 font-bold" required />
                <input name="phone" value={form.phone} onChange={handleChange} placeholder="Mobile No." className="border-2 p-2.5 rounded-xl font-black text-blue-700 outline-none focus:border-blue-500" required maxLength="10" />
              </div>
              <textarea name="address" value={form.address} onChange={handleChange} placeholder="Address" className="w-full border-2 p-2.5 rounded-xl h-20 outline-none focus:border-blue-500 resize-none font-medium" required />
            </section>
