...
export default function BulkStudentImport() {
  ...
  useEffect(() => {
    const fetchMapping = async () => {
      const snap = await getDoc(doc(db, "school_config", "master_data"));
      if (snap.exists()) {
        setSubjectMapping(snap.data().mapping || {});
      }
    };
    fetchMapping();
  }, []);
  ...
  const handleFileUpload = async (e) => {
    ...
    // 🔥 Collect unique sessions & classes
    const sessions = [...new Set(excelData.map(r => r.session || "2025-26"))];
    const classes = [...new Set(excelData.map(r => r.className))];
    ...
    for (const row of excelData) {
      try {
        ...
        const session = row.session || "2025-26";
        ...
        const regNo = regTracker[session].toString();
        const rollNumber = rollTracker[className][session].toString();

        const autoSubjects = subjectMapping[className] || [];
        ...
        const sDoc = await addDoc(collection(db, "students"), {
          ...
          transportFees: Number(row.transportFees) || 0,
          isTransferStudent: row.isTransferStudent === true,
          pnrNumber: row.pnrNumber || "",
          photoURL: "",
          parentId,
          subjects: autoSubjects,
          attendance: createAttendance(),
          fees: createFeesStructure(session),
          currentBalance: 0,
          docAadhaar: row.docAadhaar === true,
          docMarksheet: row.docMarksheet === true,
          docTC: row.docTC === true,
          docPhoto: row.docPhoto === true,
          fcmTokens: [],
          deletedAt: null,
          createdAt: serverTimestamp()
        });
